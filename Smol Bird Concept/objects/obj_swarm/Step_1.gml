...
if (keyboard_check_pressed(vk_space) && distance_to_object(obj_player) < 300)  { 
	...
	if !instance_exists(obj_cooldown) {
		xOpposite = (obj_swarm.x + (obj_swarm.x - obj_player.x)*100/distance_to_object(obj_player));
		yOpposite = (obj_swarm.y + (obj_swarm.y - obj_player.y)*100/distance_to_object(obj_player));
		pushPath = path_add();
